...
if __name__ == "__main__":
    ...
    for i in range(13):
        crop.crop(256*i, 256*(i+1), 256*i, 256*(i+1), dem=dem, lulc=lulc, rain_ant=rain_ant, rain=rain, soil=soil, out_dir=f"{i}")
        static_maps.static_maps(in_dir=f"{i}", out_dir=f"{i}")
        slam.slam(ligne=100, colonne=100, r=0, coef_cercle=0.005, cst=True, coef_pluie=1, in_dir=f"{i}", static_maps_dir=f"{i}", out_dir=f"{i}")